...
def detect_floor_y_position(screen, screen_w):
    ...
    if contours:
        ...
        if max_w_contour is not None:
            ...
            if screen_w - w < 50:   # Floor width should be close to screen width
                return y + h - 5
    else:
        return None
...
def action_function(self, screen, game_FPS, counter, time_ms):
    ...
    for p in pipes:
        ...
        # Draw bounding boxes
        cv2.rectangle(screen, (x, y), (x + w, y + h), (0, 255, 255), 4)
        cv2.putText(screen, f"({x},{y})", (x, p.syb - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
        # Draw safe zones
        cv2.line(screen, (x, p.syt), (x + w, p.syt), (0, 0, 255), 4)
        cv2.line(screen, (x, p.syb), (x + w, p.syb), (0, 0, 255), 4)

    # Draw the floor square
    if floor_y is not None:
        cv2.rectangle(screen, (0, floor_y), (self.width, self.height), (0, 0, 255), 2)
